...
fig = px.pie(df_selection, values='number_of_vehicles', names='accident_severity', title='Number of vehicles by accident severity')
st.plotly_chart(fig, use_container_width=True)

fig = px.pie(df_selection, values='number_of_vehicles', names='sex_of_driver', title = 'Number of vehicles by sex of driver')
st.plotly_chart(fig, use_container_width=True)

fig = px.pie(df_selection, values='number_of_vehicles', names='age_band_of_driver', title = 'Number of vehicles by age of driver')
st.plotly_chart(fig, use_container_width=True)

100*df_selection.pivot_table('severity', index='day_of_week', 
columns='year', aggfunc='sum')/df_selection.pivot_table('severity', index='day_of_week', columns='year', aggfunc='sum').sum()

100*df_selection.pivot_table('severity', index='month', columns='year',
 aggfunc='sum')/df_selection.pivot_table('severity', index='month', columns='year', aggfunc='sum').sum()
